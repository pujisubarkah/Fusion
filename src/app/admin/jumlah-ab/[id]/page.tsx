...
export default function DetailAnalisBangkomPage() {
	const params = useParams();
	const idParam = params?.id;
	const id = Array.isArray(idParam) ? idParam[0] : idParam || "";
	const [data, setData] = useState<AnalisBangkom | null>(null);
	const [loading, setLoading] = useState(true);
	const [error, setError] = useState("");
	const [formData, setFormData] = useState<AnalisBangkom | null>(null);
	...
	useEffect(() => {
		...
		// Fetch analis bangkom data and all dropdown options
		Promise.all([
			fetch(`/api/analis-bangkom/${id}`),
		...
		.then(async ([analisBangkomRes, pendidikanRes, instansiRes, jenjangRes, jenisKelaminRes, golonganRes, jalurRes]) => {
			if (!analisBangkomRes.ok) throw new Error("Gagal fetch data analis bangkom");
			...
			const analisBangkomData = await analisBangkomRes.json();
			...
			setData(analisBangkomData);
			setFormData(analisBangkomData);
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		try {
			const response = await fetch(`/api/analis-bangkom/${id}`, {
				method: 'PUT',
				headers: {
			...
			<div className="min-h-screen bg-gray-50 relative overflow-hidden">
				...
				<div className="p-8">
					...
					<div className="max-w-md mx-auto bg-white rounded-xl shadow-sm p-8 border border-gray-200 text-center">
						...
						<p className="text-gray-600 mb-4">{error || "Analis Bangkom yang Anda cari tidak tersedia dalam sistem."}</p>
						<Link 
							href="/admin/jumlah-ab"
		...
		<div className="min-h-screen bg-gray-50 relative overflow-hidden">
				...
				<div className="fixed top-4 right-4 z-50 animate-fade-in">
					<div className="bg-emerald-500 text-white px-6 py-4 rounded-xl shadow-lg border border-emerald-400/20 flex items-center gap-3">
						...
						<div>
							...
							<div className="text-sm opacity-90">Data analis bangkom telah diperbarui</div>
						</div>
					</div>
				</div>
			)}
			...
			<div className="p-8 flex flex-col items-center relative">
						...
						<div className="mb-8 text-center">
							<div className="flex items-center justify-center gap-3 mb-4">
								...
								<h1 className="text-4xl font-black text-blue-600">
									💼 Detail Profil Analis Bangkom
								</h1>
								<div className="w-3 h-3 bg-blue-500 rounded-full"></div>
							</div>
							<p className="text-gray-600 text-lg">Kelola dan perbarui informasi analis pengembangan kompetensi secara real-time</p>
						</div>
				...
				<div className="w-full space-y-3">
					...
					<div className="bg-blue-50 rounded-xl p-4 border border-blue-200 hover:shadow-sm transition-all duration-300 cursor-pointer">
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-2 text-blue-700">
								<FaLaptopCode className="text-lg" />
								<span className="font-bold text-sm">Spesialisasi</span>
							</div>
							<span className="text-blue-900 font-black text-xs text-right">Bangkom</span>
						</div>
					</div>
					{/* Status Card */}
									...
									<div className="flex items-center justify-between mb-6">
										<div className="flex items-center gap-3">
											...
											<h2 className="text-2xl font-bold text-blue-600">
												{isEditing ? 'Mode Edit Data' : 'Detail Data Analis Bangkom'}
											</h2>
										</div>
										<button
											type="button"
